...
// TLS
use rustls::{self, ClientConnection};
...
pub struct Config<'ws, T: Clone> {
    ...
    pub data: Option<T>,
    pub protocols: Option<&'ws[&'ws str]>,
}
...
pub struct WSClient<'ws, T: Clone> {
    ...
    acceptable_protocols: Option<&'ws [&'ws str]>,
...
impl<'ws, T> WSClient<'ws, T> where T: Clone {
    ...
    pub fn init(&mut self, host: &'ws str, port: u16, path: &'ws str, config: Option<Config<'ws, T>>) {
        self.host = host;
        self.port = port;
        self.path = path; 
    ...
    fn start_init(&mut self) -> WebSocketResult<()> {
        ...
        println!("Server name: {}", server_name.to_str());
        let conn = rustls::ClientConnection::new(Arc::new(config), server_name).unwrap();
        
        let stream: rustls::StreamOwned<rustls::ClientConnection, TcpStream> = rustls::StreamOwned::new(conn, socket);
    ...
    fn handle_send_frame(&mut self, frame: Box<dyn Frame>) -> WebSocketResult<()> {
        ...
        if sent && kind == FrameKind::Control && self.connection_status == ConnectionStatus::SERVER_WANTS_TO_CLOSE {
            ...
            self.stream.as_mut().unwrap().sock.shutdown(Shutdown::Both)?;
            self.stream = None;
    ...
    fn try_write(&mut self, bytes: &[u8]) -> WebSocketResult<bool> {
        ...
        if res.is_err(){
            ...
            if error.kind() == ErrorKind::WouldBlock { 
                ...
                println!("Error: {:?}", error);
                println!("message: {}", String::from_utf8(Vec::from(bytes)).unwrap());
    ...
    fn drop(&mut self) {
        if self.connection_status != ConnectionStatus::NOT_INIT &&
            ...
            self.stream.is_some() {
                ...
                let _ = self.stream.as_mut().unwrap().sock.shutdown(Shutdown::Both); // Ignore result from shutdown method.
            }
        }
}
